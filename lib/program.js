...
export class Program extends BaseWrapper {
    ...
    setupVertexAttributes(/** @type {import('./vertex-schema').SchemaDesc} */schema) {
        ...
        schema.items.forEach((item) => {
            ...
            if (!attr) {
                const message = `attribute "${item.name}" is unknown`;
                this._logger.error(message);
                throw new Error(message);
            }
    ...
    setUniform(/** @type {string} */name, /** @type {number} */value) {
        ...
        if (!attr) {
            const message = `uniform "${name}" is unknown`;
            this._logger.error(message);
            throw new Error(message);
        }
        const setter = uniformSetters[attr.type];
        if (!setter) {
            const message = `uniform "${name}" setter is not found`;
            this._logger.error(message);
            throw new Error(message);
        }
