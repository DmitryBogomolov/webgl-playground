...
export function parseSchema(/** @type {FieldDesc[]} */fields, /** @type {ParseSchemaOptions} */options = {}) {
    ...
    fields.forEach((field, i) => {
        if (!field.name) {
            raiseError(logger, `item ${i} "name" is not defined`);
        }
        /** @type {MetaDesc} */
        ...
        if (!(meta.bytes > 0)) {
            raiseError(logger, `item "${meta.name}" type name is not valid`);
        }
        if (!(1 <= meta.size && meta.size <= 4)) {
            raiseError(logger, `item "${meta.name}" type size is not valid`);
        }
