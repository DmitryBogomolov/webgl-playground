...
export class Primitive extends BaseObject {
    ...
    setup(config: PrimitiveConfig): void {
        if (!config) {
            throw this.logger.error('setup: config not defined');
        }
    ...
    setVertexData(vertexData: BufferSource | number): void {
        this.logger.info('set_vertex_data({0})', isBufferSource(vertexData) ? vertexData.byteLength : vertexData);
        this._runtime.bindArrayBuffer(this._vertexBuffer);
    ...
    setIndexData(indexData: BufferSource | number): void {
        this.logger.info('set_index_data({0})', isBufferSource(indexData) ? indexData.byteLength : indexData);
        try {
    ...
    updateVertexData(vertexData: BufferSource, offset: number = 0): void {
        this.logger.info('update_vertex_data(data={0}, offset={1})', vertexData.byteLength, offset);
        const gl = this._runtime.gl();
        this._runtime.bindArrayBuffer(this._vertexBuffer);
    ...
    updateIndexData(indexData: BufferSource, offset: number = 0): void {
        this.logger.info('update_index_data(data={0}, offset={1})', indexData.byteLength, offset);
        const gl = this._runtime.gl();
        // Vertex array object must be bound because element array binding is part of its state.
        this._runtime.bindVertexArray(this._vao);
    ...
    updateIndexRange(range: PrimitiveIndexRange): void {
        if (!range) {
            throw this.logger.error('update_index_range: range not defined');
        }
        const { indexOffset, indexCount } = range;
        if (indexOffset! < 0 || indexCount! < 0) {
            throw this.logger.error('update_index_range: {0} - bad values', range);
        }
        this.logger.info('update_index_range({0})', range);
        if (indexOffset! > 0) {
    ...
    setProgram(program: Program | null): void {
        ...
        this.logger.info('set_program({0})', prog);
        this._program = prog;
    }
    ...
    render(): void {
        ...
        if (this._program === EMPTY_PROGRAM) {
            throw this.logger.error('render: cannot render without program');
        }
